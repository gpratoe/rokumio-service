...
ARCH="${1:-arm64}"
case "$ARCH" in
  arm64)  ABI="arm64-v8a";    TRIPLE="aarch64-linux-android"; HOST="aarch64-linux"; FFARCH="aarch64"; FFCU="generic";  CFLAGS="-march=armv8-a" ;;
  arm)    ABI="armeabi-v7a";  TRIPLE="armv7a-linux-androideabi"; HOST="arm-linux";  FFARCH="arm";      FFCU="armv7-a";  CFLAGS="-march=armv7-a -mfloat-abi=softfp -mfpu=neon" ;;
  x86_64) ABI="x86_64";       TRIPLE="x86_64-linux-android"; HOST="x86_64-linux"; FFARCH="x86_64";   FFCU="generic";  CFLAGS="" ;;
  *) echo "Unknown arch: $ARCH (use arm64|arm|x86_64)" >&2; exit 1 ;;
esac
...
if [ ! -f "$X264_SRC/configure" ]; then
  echo "ERROR: x264 source checkout failed." >&2; exit 1
fi
...
(
  ...
  ./configure \
    ...
    --extra-asflags="-fPIC" \
    --disable-asm \
    --enable-static --disable-shared --disable-opencl --disable-cli
